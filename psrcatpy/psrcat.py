            ...
            if (len(Cols) > 6):
               ...
               while(ind < len(Cols) - 2):
                 if (len(list(filter(bool, Cols[ind].split(" ")))) !=0):
                       ...
                       self._params.append(params.Params(list(filter(bool,Cols[ind].split(" ")))[1], list(filter(bool,Cols[ind].split(" ")))[3], list(filter(bool,Cols[ind].split(" ")))[6], list(filter(bool, Cols[ind].split(" ")))[9], list(filter(bool, Cols[ind].split(" ")))[12], list(filter(bool, Cols[ind].split(" ")))[15], self.offset))
                       ind += 1
                 else:
                       ind += 1
            parser = json_parser.JSONParser(self._params)
